<?php
	...
	function Addchairman(){
		...
		if(isset($_POST['chairmanbtn'])){
			...
			$query = mysqli_query($db,"SELECT * FROM chairman WHERE 
					sname = '$sname' AND fname = '$fname' AND  mname = '$mname' AND username = '$username' ")
					or die(mysqli_error());
			...
			if(mysqli_num_rows($query)>0){
				...
				$insert = mysqli_query($db,"INSERT INTO chairman 
					(sname, fname, mname, username, password, gender, userlvl, kindofuser, image) VALUES 
					('$sname','$fname','$mname','$username','$pass','$gender','$chairman','Chairman','default.png')" ) or die(mysqli_error());
				if($insert){
					?>
				...
				if ($fileError === 0) {
					if ($fileSize<50000000) {
						...
						if ($sql) {
			              ...
			              echo "<script>alert('Success Upload!');location.href='index.php';</script>";
	...
	function calendar(){
		...
		if (isset($_POST['calendar'])) {
			...
			if($query){
				echo "<script>alert('Success Add Calendar');location.href='index.php';</script>";
			}else{
	...
	function addtext(){
		...
		if (isset($_POST['wew'])) {
			...
			$sqlQuery = mysqli_query($db,"INSERT INTO news (news,date) VALUES
			 ('$text',NOW()) ");
			if ($sqlQuery) {
				echo "<script>alert('Success Post! News');location.href</script>";
			}else{
				echo "<script>alert('Error');</script>";
